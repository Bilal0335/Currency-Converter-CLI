...
let api =
  "https://v6.exchangerate-api.com/v6/c12869655c38e1665b41d5ec/latest/USD";
...
let userMoney = await inquirer.prompt([
  {
    ...
    validate: (value) =>
      !isNaN(Number(value)) ? true : "Please enter a valid amount",
  },
]);
...
console.log(
  `${chalk.bold.yellowBright(userMoney.rupees)} ${chalk.bold.green(
    firstCountry.name
  )} ${chalk.bold.gray("=")} ${chalk.bold.yellowBright(
    convertedrate.toFixed(3)
  )} ${chalk.bold.green(secondCountry.name)}`
);
